...
S21Matrix S21Matrix::MinorMatrix(const S21Matrix &other, int rows, int cols) {
  if (rows < 0 || cols < 0) {
    throw out_of_range("Errors:");
  }
  ...
  for (int i = 0; i < other.rows_; ++i) {
    for (int j = 0; j < other.cols_; ++j) {
      if (i != rows && i != cols) {
        if (i > rows && j < cols) {
          result.matrix_[i - 1][j] = other.matrix_[i][j];
        } else if (i < rows && j > cols) {
          result.matrix_[i][j - 1] = other.matrix_[i][j];
        } else if (i > rows && j > cols) {
          result.matrix_[i - 1][j - 1] = other.matrix_[i][j];
        } else {
...
S21Matrix S21Matrix::CalcComplements() {
  ...
  if (this->rows_ == 1 && this->cols_ == 1) {
    result.matrix_[0][0] = result.matrix_[0][0];
  } else {
    for (int i = 0; i < result.rows_; ++i) {
      for (int j = 0; j < result.cols_; ++j) {
        S21Matrix temp = MinorMatrix(*this, i, j);
        result.matrix_[i][j] = temp.Determinant() * pow(-1.0, (i + j));
      }
    }
  }
  return result;
...
double S21Matrix::Determinant() {
  ...
  if (this->rows_ == 1 && this->cols_ == 1) {
    ...
    dt = (this->matrix_[0][0] * this->matrix_[1][1]) -
         (this->matrix_[1][0] * this->matrix_[0][1]);
  } else {
    for (int j = 1; j < this->cols_; ++j) {
      S21Matrix tmp = MinorMatrix(*this, 0, j - 1);
      dt += pow(-1.0, 1.0 + (double)j) * matrix_[0][j - 1] * tmp.Determinant();
    }
  }
  return dt;
}
